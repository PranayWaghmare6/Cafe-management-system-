...
function renderList(containerId, list, type) {
    ...
    if (list.length === 0) {
        ...
        const label = type === "pending" ? "No Pending Orders" : "No Completed Orders";
        container.innerHTML = `
  ...
  `).join("");
    ...
    <div class="order-card ${isPending ? "" : "completed"}"
      ...
      <div class="order-meta">
        ...
        <span class="meta-chip">🕒 ${order.orderTime}</span>
      </div>
...
function switchTab(tab) {
    ...
    if (completedPanel) completedPanel.classList.toggle("hidden", tab !== "completed");
